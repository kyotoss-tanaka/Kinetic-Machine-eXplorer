...
namespace UnityEditor.Build.Pipeline
{
    ...
    public static class ContentPipeline
    {
        ...
        internal static bool CanBuildPlayer(BuildTarget target, BuildTargetGroup targetGroup, IBuildWindowExtension buildWindowExtension)
        {
            ...
            // PATCHED (Unity 6.3 Web): the legacy IBuildWindowExtension is null for the
            // renamed "Web" target, so EnabledBuildButton() reports a false negative and
            // blocks Addressables content builds. Build-target support is already verified
            // above, so it is safe to allow the build here.
