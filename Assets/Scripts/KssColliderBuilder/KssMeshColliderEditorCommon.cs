...
using SimpleCountDownLatch = KssColliderBuilderEditorCommon.SimpleCountDownLatch;
...
public class KssMeshColliderEditorCommon
{
            ...
            for (; transform != null; transform = transform.parent)
            {
                ...
                if (meshFilter != null && meshFilter.sharedMesh != null)
                {
                    return new MeshCache(
                        KssColliderBuilderEditorCommon.GetMesh(meshFilter),
                        KssColliderBuilderEditorCommon.GetMaterials(meshFilter));
                }
                ...
                {
                    return new MeshCache(
                        KssColliderBuilderEditorCommon.GetMesh(skinnedMeshRnederer),
                        KssColliderBuilderEditorCommon.GetMaterials(skinnedMeshRnederer));
                }

                if (KssColliderBuilderEditorCommon.IsRootTransform(transform))
                {
                    return null;
                }
            }
...
//            Debug.LogError("");
    ...
    public static SplitMesh[] MakeSplitMeshesByPolygon(MeshCache meshCache, SplitMesh splitMesh, float splitPolygonNormalAngle)
    {
        ...
        for (int triangle = 0; triangle < triangles.Length; triangle += 3)
        {
            if (!usedTrianglesAll[triangle / 3])
            {
                ...
                KssColliderBuilderEditorCommon.CompactMesh compactMesh = KssColliderBuilderEditorCommon.MakeCompactMesh(resplitMesh.vertices, resplitMesh.triangles);
                if (compactMesh != null)
                {
    ...
    public static int CleanupChildSAMeshColliders(GameObject parentGameObject, bool cleanupModified)
    {
        ...
        List<Transform> childTransformList = KssColliderBuilderEditorCommon.GetChildTransformList(parentGameObject.transform);
    ...
    public static void RegistReducerTask(List<ReducerTask> reducerTasks, SAMeshCollider meshCollider)
    {
        ...
        if (reducerProperty.shapeType == ShapeType.Mesh)
        {
            ...
            {
                reduceMode = (int)KssColliderBoxReducer.ReduceMode.Mesh;
            }
            else
            {
                reduceMode = (int)KssColliderBoxReducer.ReduceMode.BoxMesh;
            }
        }
        else
        {
            reduceMode = (int)KssColliderBoxReducer.ReduceMode.Box;
        }
    ...
    public static void ReduceMeshCollider(ReducerTask reducerTask)
    {
        ...
        reducerTask.reducerParams.lines = KssColliderBuilderEditorCommon.TriangleToLineIndices(reducerTask.splitMesh.triangles);
        ...
        bool isDebug)
    {
        ...
        KssColliderBuilderEditorCommon.AddRigidbody(meshCollider.gameObject, rigidbodyProperty);
        ...
        { // Logging.
            ...
            KssColliderBuilderEditorCommon.DumpHierarchy(str, gameObject);
...
//                Debug.LogWarning(str.ToString());
    ...
    public static ReducerResult PostfixReducerVertices(ReducerParams reducerParams, Vector3[] vertices, int[] triangles)
    {
        ...
        KssColliderBoxReducer reducer = new KssColliderBoxReducer();
        reducer.reduceMode = KssColliderBoxReducer.ReduceMode.Box;
        ...
        Quaternion reduceRotation = KssColliderBoxReducer.InversedRotation(reducer.reducedRotation);
        Vector3 reducedCenter = reducer.reducedCenter;

        Matrix4x4 transform = KssColliderBoxReducer._TranslateRotationMatrix(-reducedCenter, reduceRotation);
        ...
        if (reducerParams.reducerProperty.scale != Vector3.one)
        {
            ...
            for (int i = 0; i < transformVertices.Length; ++i)
            {
                transformVertices[i] = KssColliderBoxReducer.ScaledVector(transformVertices[i], scale);
            }
        }
    ...
    {
        KssColliderBuilderEditorCommon.CompactMesh compactMesh = KssColliderBuilderEditorCommon.MakeCompactMesh(
            vertices, triangles);
        if (compactMesh != null)
        {
    ...
    public static ReducerResult Reduce(ReducerParams reducerParams)
    {
                    ...
                    reducerParams.reducerProperty.meshType == MeshType.ConvexHull)
        {
            ...
            KssColliderHullReducer.HullDesc hullDesc = new KssColliderHullReducer.HullDesc();

            hullDesc.mFlags = KssColliderHullReducer.HullFlag.QF_TRIANGLES;
            ...
            KssColliderHullReducer.HullResult hullResult = new KssColliderHullReducer.HullResult();
            KssColliderHullReducer.HullLibrary hullLibrary = new KssColliderHullReducer.HullLibrary();

            KssColliderHullReducer.HullError hullError = hullLibrary.CreateConvexHull(hullDesc, hullResult);
            if (hullError == KssColliderHullReducer.HullError.QE_FAIL)
            {
                return MakeReduceMeshResult(reducerParams, vertices, triangles);
            }
        ...
        else
        {
            ...
            KssColliderBoxReducer reducer = new KssColliderBoxReducer();
            reducer.reduceMode = (KssColliderBoxReducer.ReduceMode)reducerParams.reduceMode;
            ...
            reducer.sliceMode = (KssColliderBoxReducer.SliceMode)(int)reducerProperty.sliceMode;
    ...
    public static void Reduce(List<ReducerTask> reducerTasks, bool isDebug)
    {
        ...
        for (int i = 0; i < reducerTasks.Count; ++i)
        {
            KssMeshColliderEditorCommon.CreateCollider(
