...
public class MPS2_3AS : ParallelLink
{
    ...
    protected override void ModelRestructProcess()
    {
        ...
        var allArm2 = children.Where(d => d.name.Contains("第二アーム")).Select(d => d.gameObject).ToList();
        ...
        for (var i = 0; i < AXIS_MAX; i++)
        {
            ...
            tmpArm1.transform.localPosition = new Vector3(fH[i] / 1000 * Mathf.Sin(ARM_RAD_OFFSET[i]), -baseObject.transform.localPosition.y, fH[i] / 1000 * Mathf.Cos(ARM_RAD_OFFSET[i]));
        ...
        tmpPlate.transform.localPosition = new Vector3(0, 0, 0);
