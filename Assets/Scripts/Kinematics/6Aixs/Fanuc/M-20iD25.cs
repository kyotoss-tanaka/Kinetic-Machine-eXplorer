...
public class M_20iD25: Kinematics6D
{
    #region 変数
    [SerializeField]
    ...
    */
    ...
    public override void SetTarget(float x, float y, float z, float rx, float ry, float rz)
    {
        ...
        arm6.localEulerAngles = new Vector3(0, ang6.y, ang6.z + rz);
    }
    ...
    /// <summary>与えた腕Transform群を J1..J6(度) の姿勢にする（SetTarget と同じ式）。ゴースト共用。
    /// arm6 は SetTarget と同じく (0, ang6y, ang6z + J6)＝z軸回り（本機は arm6 が z 回転・初期オフセット ang6）。</summary>
    public static void ApplyArmPose(Transform a1, Transform a2, Transform a3, Transform a4, Transform a5, Transform a6,
                                    float ang1x, float ang6y, float ang6z, double[] j, bool ros2)
    {
        if (j == null || j.Length < 6)
        {
            return;
        ...
        if (a6) { a6.localEulerAngles = new Vector3(0f, ang6y, ang6z + (float)j[5]); }
    }
    ...
    public override GameObject CreateGhost()
    {
        ...
        // 腕参照（複製内を名前で探索・ModelRestructProcess の arm1..arm6 と同じ名前で対応させる）。
        //   ★CRX のコピー元は J2BASE 等だったが、M-20iD/25 モデルの実名は J2^01N1 等。ここが CRX名のままだと
        //     g1..g6 が全て null になり、ApplyArmPose が何もせず「ゴーストが動かない」不具合になる。
        var ch = ghost.GetComponentsInChildren<Transform>(true).ToList();
        g1 = ch.Find(d => d.name.Contains("J2^01N1"));       // arm1
        g2 = ch.Find(d => d.name.Contains("J3^01N1"));       // arm2
        g3 = ch.Find(d => d.name.Contains("J4^01N1"));       // arm3
        g4 = ch.Find(d => d.name.Contains("J3_ARM^01N1"));   // arm4
        g5 = ch.Find(d => d.name.Contains("J5_part4^01N1")); // arm5
        g6 = ch.Find(d => d.name.Contains("J6FLANGE^01N1")); // arm6
    ...
    public override void PoseGhostDeg(double[] j16)
    {
        ...
        ApplyArmPose(g1, g2, g3, g4, g5, g6, ghostAng1x, ang6.y, ang6.z, j16, GlobalScript.useRos2);
    }
    ...
    /// <summary>機種キー（robot_id=m20_25_18d_N の生成・機種別既定の索引）。ROS2 robot_map と一致させる規約。</summary>
    public override string ModelKey => "m20_25_18d";
    ...
    public override double[] GetCurrentJointsDeg()
    {
        ...
        float j6 = Mathf.DeltaAngle(ang6.z, arm6.localEulerAngles.z);   // arm6.z = ang6.z + J6（SetTarget と同規約）
